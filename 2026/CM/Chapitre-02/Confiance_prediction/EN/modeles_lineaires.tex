...
\begin{frame}{Variance of $\hat{\beta}$}
	...
	\end{equation}
	\pause
	The standard deviation (standard error) $\ET[\hat{\beta}] \in \mathcal{M}_{d+1, 1}(\mathbb{R}^+)$ can then be easily deduced:
	\begin{equation}
...
\begin{frame}{Connection to One-Dimensional Estimation}
	In the previous section, we saw that we could estimate the standard error from the standard deviation of an observation:
	\begin{equation}
...
\begin{frame}{Prediction Space for the Class}
	In a binary classification problem, $\hat{p}(x^{(n+1)})$ represents $P(y^{(n+1)} = 1 \mid x^{(n+1)})$. \\
	Based on the values of $\hat{p}_{\text{lower}}$ and $\hat{p}_{\text{upper}}$, we can define the prediction space at level $1 - \alpha$ as follows:
	\begin{equation}
		C(x^{(n+1)}) = 
		\begin{cases}
